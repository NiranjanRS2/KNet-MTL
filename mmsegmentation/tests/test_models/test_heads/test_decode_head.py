...
def test_decode_head():
    ...
    with pytest.raises(AssertionError):
        # default input_transform doesn't accept multiple inputs
        BaseDecodeHead(32, 16, num_classes=3, in_index=[-1, -2])

    with pytest.raises(AssertionError):
        # supported mode is resize_concat only
        BaseDecodeHead(32, 16, num_classes=3, input_transform='concat')

    with pytest.raises(AssertionError):
        # in_channels should be list|tuple
        BaseDecodeHead(32, 16, num_classes=3, input_transform='resize_concat')
    ...
    # test set dropout
    head = BaseDecodeHead(32, 16, num_classes=3, dropout_ratio=0.2)
    ...
    with pytest.raises(TypeError):
        ...
        BaseDecodeHead(3, 16, num_classes=3, loss_decode=['CrossEntropyLoss'])
    ...
    with pytest.raises(TypeError):
        BaseDecodeHead(3, 16, num_classes=3, loss_decode=['CrossEntropyLoss'])
    with pytest.raises(TypeError):
        BaseDecodeHead(3, 16, num_classes=3, loss_decode=0)
