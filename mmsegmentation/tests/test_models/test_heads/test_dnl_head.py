...
def test_dnl_head():
    ...
    head = DNLHead(
        in_channels=8, channels=4, num_classes=3, mode='dot_product')
    inputs = [torch.randn(1, 8, 23, 23)]
    if torch.cuda.is_available():
        head, inputs = to_cuda(head, inputs)
    outputs = head(inputs)
    ...
    head = DNLHead(in_channels=8, channels=4, num_classes=3, mode='gaussian')
    inputs = [torch.randn(1, 8, 23, 23)]
    if torch.cuda.is_available():
        head, inputs = to_cuda(head, inputs)
    outputs = head(inputs)
    ...
    head = DNLHead(
        in_channels=8, channels=4, num_classes=3, mode='concatenation')
    inputs = [torch.randn(1, 8, 23, 23)]
    if torch.cuda.is_available():
        head, inputs = to_cuda(head, inputs)
    outputs = head(inputs)
