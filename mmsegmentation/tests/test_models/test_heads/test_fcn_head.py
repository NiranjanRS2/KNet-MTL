...
def test_fcn_head():
    ...
    with pytest.raises(AssertionError):
        ...
        FCNHead(num_classes=3, num_convs=-1)
    ...
    head = FCNHead(
        in_channels=8, channels=4, num_classes=3, concat_input=False)
    ...
    inputs = [torch.randn(1, 8, 23, 23)]
    head = FCNHead(
        in_channels=8, channels=4, num_classes=3, concat_input=True)
    if torch.cuda.is_available():
        head, inputs = to_cuda(head, inputs)
    assert len(head.convs) == 2
    ...
    # test kernel_size=1
    inputs = [torch.randn(1, 8, 23, 23)]
    head = FCNHead(in_channels=8, channels=4, num_classes=3, kernel_size=1)
    if torch.cuda.is_available():
        head, inputs = to_cuda(head, inputs)
    for i in range(len(head.convs)):
    ...
    # test num_conv
    inputs = [torch.randn(1, 8, 23, 23)]
    head = FCNHead(in_channels=8, channels=4, num_classes=3, num_convs=1)
    if torch.cuda.is_available():
        head, inputs = to_cuda(head, inputs)
